public class Netstrings {
	...
	String decodedNetstring(String encodedInput){
		if (encodedInput.length() < 3) return "error"; //if the Netstring is less than 3 characters, it's either an invalid one or contains an empty string
		...
		if (controlDigits < 0) return "error"; //if the control digit is smaller than 1, then it's either not a digit or the Netstring is invalid
		String command = encodedInput.substring(semicolonIndex+1); //parse after the semicolon until the end of the string
		if (command.length() == 0) return "error"; // if it's an empty string, return "error"
